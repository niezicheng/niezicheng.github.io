...
- 加密密钥在传递给对方时可能会被拦截，导致信息可能会被篡改和窥视
...
> 解决 `RSA` 算法加密很慢问题（对称加密）

- 使用 `RSA` （非对称加密）公钥加密的**对称密钥**，然后在进行传递（RAS 加密密钥）
- 使用 `RSA` （非对称加密）私钥解密获取**对称密钥**，后面使用对称密钥进行通信
...
- 使用 `CA` 私钥加密原始信息生成 `数字签名` 并结合**原始信息** 生成证书一起发送
- 通过 `CA` 对应的公钥解密 `数字签名` 获取原始信息并与一起发送过来的原始信息比较（判断原始信息内容是否被篡改）
...
- `RFC` 指出：单用户客户端与任何服务器或代理之间的连接数不应该超过 `2` 个。一个代理与其它服务器或代码之间应该使用不超过 `2 * N` 的活跃并发连接。这是为了提高 `HTTP` 响应时间，避免拥塞（冗余的连接并不能代码执行性能的提升
